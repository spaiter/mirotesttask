...
import com.miro.api.widgets.testtask.SqlTestConfiguration;
...
@ActiveProfiles("test")
@Import(SqlTestConfiguration.class)
public class WidgetControllerSqlServiceIntegrationTest {
    ...
    public void whenManyPostRequestToWidgetsAndValidWidgetsWithDifferentZIndex_thenCorrectResponse() throws Exception {
        ...
        assertThat(widgets.get(2).getZIndex()).isEqualTo(11); //10
        assertThat(widgets.get(3).getZIndex()).isEqualTo(21); //20
        assertThat(widgets.get(4).getZIndex()).isEqualTo(22); //21
        assertThat(widgets.get(5).getZIndex()).isEqualTo(32); //30
    }
    ...
    @Test
    public void whenGetFilteredRequestsAfterWidgetDeleteRequest_thenCorrectResponse() throws Exception {
        List<WidgetResponseDTO> requests = List.of(
                new WidgetCreateRequestDTO(0, 0, 1, 1, 1),
                new WidgetCreateRequestDTO(1, 1, 2, 1, 1),
                new WidgetCreateRequestDTO(2, 2, 3, 1, 1)
        ).stream().map(request -> {
            try {
                return objectMapper.writeValueAsString(request);
            } catch (JsonProcessingException e) {
        ...
        mockMvc.perform(MockMvcRequestBuilders.delete(String.format("/widgets/%s", requests.get(1).getId()))
                .contentType(MediaType.APPLICATION_JSON))
                .andExpect(MockMvcResultMatchers.status().isOk());
